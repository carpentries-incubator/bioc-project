...
In the era of genomics, many observations are reported as ranges of coordinates
- i.e., intervals - on a genomic scale.
...
As such, individual ranges can be extracted by integer index or name like any
regular vector.

```{r}
demo_with_metadata[1]
demo_with_metadata["B"]
```
...
Each row in the object reports an overlap between one range in the query set
