...
component called `seqinfo`, which is used to store information about each
sequence that may be represented in the `seqnames` component.
...
Frequently, genomic ranges are imported from files rather than described in
...
In particular, we use the `import()` generic defined in the
`r BiocStyle::Biocpkg("BiocIO")` package and implemented in the
`r BiocStyle::Biocpkg("rtracklayer")` package as a versatile function that is
capable of recognising common file extensions and associating them with the
appropriate method for parsing each particular file format.
...
>
> Refer to the `r BiocStyle::Biocpkg("rtracklayer")` package documentation
> for the full list of methods available.
{: .callout}
...
In the example below, we extract all the transcript records that start at
position 5527147.
...
stored in a single `GRanges` object into groups that are stored in a named list
of `GRanges` objects.
...
exons, then separate those exons by transcript identifier, before finally
coercing the list of `GRanges` objects into a `GRangesList` object.

```{r}
actb_exons <- subset(actb_gtf_data, type == "exon")
actb_exons_by_transcript <- split(actb_exons, actb_exons$transcript_id)
class(actb_exons_by_transcript)
actb_exons_by_transcript <- as(actb_exons_by_transcript, "GRangesList")
...
the length of each `GRanges` object in the list.

In the latest example above, we can show the number of exons in each transcript:
