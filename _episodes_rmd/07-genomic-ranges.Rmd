...
In the following example, we import the gene model for Actin Beta (ACTB) from a small GTF file as a set of genomic ranges.

```{r}
library(rtracklayer)
actb_gtf_data <- import("data/actb.gtf")
actb_gtf_data
```
...
> species, downloaded from the [Ensembl FTP site][ensembl-ftp].
> The original file contains more than 3 millions lines and 22 metadata fields,
> from which a subset was extracted for convenience and brevity.
{: .callout}
