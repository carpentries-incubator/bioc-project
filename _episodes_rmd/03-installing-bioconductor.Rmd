...
Each time a new release is produced, the minor version (see note below) of all the packages in the Bioconductor repository is incremented by one.
This is the case for every package, even those which have not been updated since the previous release.
This version increment is essential to associated a unique version of each Bioconductor package with each release of the Bioconductor project.
...
In turn, this determines the version of Bioconductor packages to be installed by the `BiocManager::install()` function.
...
> ## Contribute!
>
> It would be great to insert here a figure illustrating the timeline of recent releases of Bioconductor and R.
> Basically, turning the table [here](https://www.bioconductor.org/about/release-announcements/) into a figure.
>
{: .callout}
